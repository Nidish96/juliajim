...
for i in eachindex(his[his.<=maximum(h)])
    local ax = Axis(fig[1, i], xlabel=L"Modal Amplitude $a$",
    ...
    local ax = Axis(fig[2, i], xlabel=L"Modal Amplitude $a$",
...
ax = Axis(fig[1, length(his)+1], xlabel=L"Modal Amplitude $a$",
    ylabel=L"Natural Frequency $\omega_n$ (rad/s)", xscale=log10);
scatterlines!(ax, As, Oms);

ax = Axis(fig[2, length(his)+1], xlabel="Modal Amplitude",
