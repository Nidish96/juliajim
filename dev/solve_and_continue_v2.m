...
function [X,Solinfo,Sol] = ...
        solve_and_continue_v2(x0,fun_residual,lam_s,lam_e,ds,varargin)
...
% Default continuation options
    ...
    while istep<=Sopt.stepmax
        ...
        if Sopt.dynamicDscale
            ...
            Dscale  = Sopt.Dscale;
            sclX    = max(ceil(log10(abs(X0(1:end-1).*Dscale(1:end-1)))));
            sclNew  = sclX-sign(sclX)*Sopt.Dcorr;
            Sopt.Dscale(1:end-1) = 10^(sclNew)*ones(size(X0(1:end-1)));    

            %% Update scaled variable values, Jacobian, reference tangent
            Xref    = Xref.*(Dscale./Sopt.Dscale);
            X0      = X0.*(Dscale./Sopt.Dscale);
            Xold    = Xold.*(Dscale./Sopt.Dscale);
        else
            Dscale = Sopt.Dscale;
        end
        % remove scaling
        J       = J*diag(1./Dscale);
        zref    = zref.*Dscale;zref=zref./norm(zref);
        %% PREDICT
        ...
        if Sopt.flag
            switch Sopt.predictor
              case 'tangent'
                [~,kk] = sort(abs(zref./max(abs(Xref.*Dscale),1e-4)),...
                              1,'descend');
